...
export default function EditEstimatePage({
  ...
  // Page load state
  const [loadingEstimate, setLoadingEstimate] = React.useState(true)
  const [notFound, setNotFound] = React.useState(false)
  const [estimateNumber, setEstimateNumber] = React.useState(0)
  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      const [custRes, pbRes, estRes, liRes] = await Promise.all([
        ...
        supabase.from('estimates').select('*').eq('id', id).eq('user_id', user.id).maybeSingle(),
        supabase.from('estimate_line_items')
          .select('id, pricebook_item_id, name, description, quantity, unit_price')
          .eq('estimate_id', id)
          .order('id'),
      ])
      ...
      setEstimateNumber(est.estimate_number ?? 0)
      ...
      setMarkupPct(est.markup_percent ? String(est.markup_percent) : '')
      setTaxPct(est.tax_percent ? String(est.tax_percent) : '')
      setDiscount(est.discount ? String(est.discount) : '')
      setNotes(est.notes ?? '')
      setInternalNotes(est.internal_notes ?? '')
      setRequirePayment(est.require_payment ?? false)
      setPaymentType(est.payment_type ?? 'full')
      setDepositPct(est.deposit_percent ? String(est.deposit_percent) : '50')
  ...
  const updateLineItem = (itemId: string, field: keyof Omit<LineItem, '_id' | 'pricebook_item_id'>, value: string | number) => {
    setLineItems((prev) => prev.map((item) => item._id === itemId ? { ...item, [field]: value } : item))
  }
  ...
  const handleSave = async (status: 'Draft' | 'Sent') => {
    ...
    // Replace all line items: delete existing, insert updated set
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto pb-28">
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          <Button variant="ghost" size="sm" asChild className="gap-1.5 -ml-2 text-muted-foreground hover:text-foreground mb-3">
          ...
          <h1 className="text-2xl font-semibold text-foreground">
            Edit {estimateNumber ? fmtEstNum(estimateNumber) : 'Estimate'}
          </h1>
          <p className="text-sm text-muted-foreground mt-0.5">Update estimate details and line items</p>
        </div>
      </div>
          ...
          <div className="relative mb-4" ref={pbDropdownRef}>
            ...
            {pbOpen && pbSearch && pbFiltered.length === 0 && (
              <div className="absolute z-10 top-full mt-1 w-full rounded-md border border-border bg-popover shadow-lg py-3 px-3 text-sm text-muted-foreground">
                No pricebook items match &ldquo;{pbSearch}&rdquo;
              </div>
            )}
          </div>
        ...
        <CardContent className="px-6 py-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="flex flex-col gap-4">
              <Field label="Markup (%)">
                <Input type="number" min="0" step="any" placeholder="0" value={markupPct} onChange={(e) => setMarkupPct(e.target.value)} />
              </Field>
              <Field label="Tax (%)">
                <Input type="number" min="0" step="any" placeholder="0" value={taxPct} onChange={(e) => setTaxPct(e.target.value)} />
              </Field>
